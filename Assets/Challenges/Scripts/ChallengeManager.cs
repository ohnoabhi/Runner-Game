...
namespace Challenges
{
    public class ChallengeManager : MonoBehaviour
    {
        ...
        [SerializeField] private Reward specialReward;

        private Challenge[] _currentChallenges;
        // private const string DateFormat = "dd-MM-yyyy hh:mm:ss tt";

        private DateTime _nextChallengeTime;

        private SpecialReward specialRewardItem;
        ...
        private void Awake()
        {
            ...
            specialRewardItem = LoadSpecialReward();
            ...
            else
            {
                ...
                else OnChallengeUpdate?.Invoke(_currentChallenges, specialRewardItem);
            }
        }
        ...
        public void UpdateChallenge(ChallengeType type, int amount = 1)
        {
            foreach (var challenge in _currentChallenges)
            {
        ...
        public void Save()
        {
            ...
            if (specialRewardItem != null)
            {
                using var specialRewardFileStream = File.Create(_specialRewardSavePath);
                binaryFormatter.Serialize(specialRewardFileStream, specialRewardItem);
                specialRewardFileStream.Close();
            }

            OnChallengeUpdate?.Invoke(_currentChallenges, specialRewardItem);
        }
        ...
        {
            if (specialRewardItem != null && !specialRewardItem.IsClaimed) return;
            specialRewardItem = new SpecialReward()
            {
                Reward = new Reward(specialReward)
            };
            Save();
        }
        ...
        {
            return Instance.specialRewardItem;
        }
    }
}